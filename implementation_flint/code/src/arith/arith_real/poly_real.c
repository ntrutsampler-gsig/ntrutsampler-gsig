...
static acb_dft_pre_t pre[PARAM_LOG_N];
...
void arith_real_setup(void) {
  for (size_t i = 1; i <= PARAM_LOG_N; i++) {
...
void arith_real_teardown(void) {
  for (size_t i = 0; i < PARAM_LOG_N; i++)
  {
...
*
* Arguments:   - poly_real res: polynomial to be zeroized (initialized)
**************************************************/
...
* Name:        poly_real_set
*
* Description: Set an initialized polynomial to another polynomial
*
* Arguments:   - poly_real res: polynomial to be set (initialized)
*              - const poly_real arg: polynomial to set
**************************************************/
void poly_real_set(poly_real res, const poly_real arg) {
  arb_poly_set(res, arg);
}
...
/*************************************************
* Name:        poly_real_copy_add_constant
*
* Description: Add a scalar to the constant coefficient of a polynomial
* 
* Arguments:   - poly_real res: polynomial to host the incremented polynomial (initialized)
*              - const poly_real arg: polynomial to be incremented
*              - const coeff_real c: scalar summand
**************************************************/
void poly_real_copy_add_constant(poly_real res, const poly_real arg, const coeff_real c) {
  arb_poly_get_coeff_arb(TMP, arg, 0);
  arb_set_d(AUX, c);
  arb_add(TMP, TMP, AUX, POLY_REAL_PRECISION);
  arb_poly_set(res, arg);
  arb_poly_set_coeff_arb(res, 0, TMP);
