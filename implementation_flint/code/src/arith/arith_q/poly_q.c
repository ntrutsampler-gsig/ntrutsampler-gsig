...
static nmod_poly_t POLY_F_MOD_QL;

#if PARAM_KH != 2
#error "Inverse modulo q only implemented for k_H = 2"
#endif
...
void arith_q_setup(void) {
  ...
  nmod_poly_init(POLY_F_MOD_QL, PARAM_QL);
  nmod_poly_set_coeff_ui(POLY_F_MOD_QL, 0, 1);
  nmod_poly_set_coeff_ui(POLY_F_MOD_QL, PARAM_N, 1);
...
*              polynomial, fails if not in I(x^{k_pi})
*
* Arguments:   - const poly_q arg: the polynomial
*
* Returns the Hamming weight of the binary polynomial, and -1 if not in I(x^{k_pi})
**************************************************/
int64_t poly_q_weight(const poly_q arg) {
  int64_t weight = 0;
  int64_t weight_k_pi = 0;
  coeff_q tmp;
  for (size_t i = 0; i < PARAM_N; i++) {
...
*              over PARAM_QL * Ring + arg, with width s_L
* 
* Arguments:   - poly_q res: the polynomial to host the Gaussian sample (initialized)
...
void poly_q_gaussian_coset_sL(poly_q res, const poly_q arg) {
  ...
  for (size_t i = 0; i < PARAM_N; i++) {
    ...
    neg_c_div_qL = - ((double)c) / PARAM_QL;
    z = SampleZ(neg_c_div_qL, PARAM_SL_DIV_QL);
    c += (PARAM_QL * z);
    poly_q_set_coeff(res, i, c);
  }
}
...
/*************************************************
* Name:        poly_q_invert_mod_q_bH
*
* Description: Compute the inverse of a polynomial in Z[x]/(q, x^n+1)
*              and modulo bH at the same time
*
* Arguments:   - poly_q inv_q: polynomial to host the inverse mod PARAM_Q
*              - poly_q inv_bH polynomial to host the inverse mod PARAM_BH
*              - const poly_q arg: polynomial to invert
**************************************************/
void poly_q_invert_mod_q_bH(poly_q inv_q, poly_q inv_bH, const poly_q arg) {
  // Flint does not handle composite moduli for xgcd
  size_t j;
  coeff_q c, c_z1, c_z2;
  nmod_poly_t arg_qL, res_qL, arg_bH, res_bH;
  fmpz_poly_t arg_qH, res_qH, tmp;
  nmod_poly_init(arg_qL, PARAM_QL);
  nmod_poly_init(res_qL, PARAM_QL);
  nmod_poly_init(arg_bH, PARAM_BH);
  nmod_poly_init(res_bH, PARAM_BH);
  fmpz_poly_init(arg_qH);
  fmpz_poly_init(res_qH);
  fmpz_poly_init(tmp);

  /* INVERSE MODULO PARAM_QL */
  // convert arg to be modulo PARAM_QL
  for (j = 0; j < PARAM_N; j++) {
    c = poly_q_get_coeff(arg,j);
    c %= PARAM_QL;
    c += (c >> (sizeof(coeff_q)*8-1)) & PARAM_QL;
    ASSERT_DEBUG(c >= 0, "Don't give me too much negativity!");
    nmod_poly_set_coeff_ui(arg_qL, j, c);
  }
  nmod_poly_invmod(res_qL, arg_qL, POLY_F_MOD_QL);

  /* INVERSE MODULO PARAM_BH */
  ...
  for (j = 0; j < PARAM_N; j++) {
    ...
    poly_q_set_coeff(TMP, j, c);
  }
  nmod_poly_invmod(res_bH, arg_bH, POLY_F_MOD_BH);

  /* Computing product of res_bH and arg_bH in R, must be in centered representation */
  for (j = 0; j < PARAM_N; j++) {
    c = nmod_poly_get_coeff_ui(arg_bH, j);
    c = c - ((~((c - PARAM_BH/2) >> (sizeof(coeff_q)*8-1))) & PARAM_BH);
    fmpz_poly_set_coeff_si(arg_qH, j, c);
    c = nmod_poly_get_coeff_ui(res_bH, j);
    poly_q_set_coeff(inv_bH, j, c);
    c = c - ((~((c - PARAM_BH/2) >> (sizeof(coeff_q)*8-1))) & PARAM_BH);
    fmpz_poly_set_coeff_si(res_qH, j, c);
  }

  fmpz_poly_mul(tmp, arg_qH, res_qH);
  for (j = 0; j < PARAM_N; j++) {
    c_z1 = fmpz_poly_get_coeff_si(tmp, (signed long)j);
    c_z2 = fmpz_poly_get_coeff_si(tmp, (signed long)j + PARAM_N);
    fmpz_poly_set_coeff_si(tmp, (signed long)j, c_z1 - c_z2);
    fmpz_poly_set_coeff_ui(tmp, (signed long)j + PARAM_N, 0);
  }
  fmpz_poly_neg(arg_qH, tmp);
  fmpz_poly_set_coeff_si(arg_qH, (signed long)0, (signed long)2 + fmpz_poly_get_coeff_si(arg_qH, (signed long)0)); // holds 1 + bH.quot
  ...
  // tmp holds inverse of arg modulo b_H^2

  /* Recombine CRT */
  for (j = 0; j < PARAM_N; j++) {
    c_z1 = fmpz_poly_get_coeff_si(tmp, j);
    c_z1 %= PARAM_QH;
    /*c_z1 += (c_z1 >> (sizeof(coeff_q)*8-1)) & PARAM_QH;
    ASSERT_DEBUG(c >= 0, "Don't give me too much negativity!");*/
    c = (coeff_q)nmod_poly_get_coeff_ui(res_qL, j);
    poly_q_set_coeff(inv_q, j, (coeff_q)c_z1 * PARAM_QL_MUL_QL_INVMOD_QH + c * PARAM_QH_MUL_QH_INVMOD_QL);
  }

  poly_q_mul(TMP,inv_q,arg);
  ASSERT_DEBUG(nmod_poly_is_one(TMP), "Polynomial inversion did not work correctly.");
  poly_q_mul(TMP,inv_bH,arg);
  poly_q_mod_bH(AUX, TMP);
  ASSERT_DEBUG(nmod_poly_is_one(AUX), "Polynomial inversion did not work correctly.");

  nmod_poly_clear(arg_qL);
  nmod_poly_clear(res_qL);
  nmod_poly_clear(arg_bH);
  nmod_poly_clear(res_bH);
  fmpz_poly_clear(arg_qH);
  fmpz_poly_clear(res_qH);
  fmpz_poly_clear(tmp);
}
