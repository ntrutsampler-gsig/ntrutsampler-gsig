...
void perturbation_sampler(
  ...
  for (i = 0; i < PARAM_KH; i++) {
    poly_real_zero(tmp1);
    poly_real_samplefz(tmp2, g[i], tmp1);

    poly_q_from_poly_real(p2[i], tmp2);

    poly_real_mul(tmp1, tmp2, f_prime[i]);
    poly_real_add(c, c, tmp1);
  }
  poly_real_samplefz(tmp2, s_p, c);
  poly_q_from_poly_real(p1, tmp2);
