...
#-- Import --#
from math import sqrt, log2, floor, ceil, pi, exp, e
import sys, os
...
def find_bkz_blocksize(delta_0: float):
    ...
    if delta_0 < 1.00057456569590:
        print('[!] Root Hermite Factor smaller than 1.000574: Security over 1024 bits in pessimistic Core-SVP model')
        return 4935
    b = 36
...
def core_svp_cost(b: int, it=1, cost_model='sieving'):
    ...
    if cost_model == 'sieving':
        csec, qsec = log2(sqrt(3/2)) * b + log2(it), 0.257 * b + log2(it)
    elif cost_model == 'realistic_sieving':
        csec, qsec = log2(sqrt(3/2)) * b + log2(it) + 16.4, 0.257 * b + log2(it) + 16.4
...
def estimate_SIS(n: int, m: int, q: int, beta: float, betainf=-1, cost_model='sieving', it=1, estimator=False, rough=True):
    """
    ...
    if estimator:
        if rough:
            results = SIS.estimate.rough(SIS.Parameters(n=n, q=q, length_bound=beta, m=m, norm=2))
        else:
            results = SIS.estimate(SIS.Parameters(n=n, q=q, length_bound=beta, m=m, norm=2))
...
def estimate_ISIS(n: int, m: int, q: int, beta: float, cost_model='sieving', it=1, estimator=False, rough=True):
    """
    ...
    """
    if beta < q:
        subdim, delta_0, b, csec, qsec = estimate_SIS(n=n, m=m, q=q, beta=beta, cost_model=cost_model, it=it, estimator=estimator, rough=rough)
...
def estimate_LWE(n: int, q: int, Xs, Xe, m: int, it=1, cost_model='sieving', rough=True):
    ...
    - output: 
        ...
        (flt)   qsec        -- Quantum bit security
    """
    deny_list = ("bkw", "arora-gb")
    if m >= n ** 2:
        print('[Warning] Algebraic attacks may apply')
        deny_list = ()
    # Running estimator
    if rough:
        results = LWE.estimate.rough(LWE.Parameters(n=n, q=q, Xs=Xs, Xe=Xe, m=m))
    else:
        results = LWE.estimate(LWE.Parameters(n=n, q=q, Xs=Xs, Xe=Xe, m=m), deny_list=deny_list)

    b = min([results[alg]['beta'] for alg in results.keys() if 'beta' in results[alg]])
    delta_0 = rhf_from_bkz(b)
    csec, qsec = core_svp_cost(b, it=it, cost_model=cost_model)
    return b, delta_0, csec, qsec

def estimate_NTRU(n: int, q: int, Xs, Xe, m: int, ntru_type="circulant", it=1, cost_model='sieving', rough=True):
    """
    Estimate the Core-SVP hardness of NTRU with dimension n, samples m, modulus q, secret 
    distribution Xs, error distribution Xe.
    - input: 
        (int)   n           -- LWE dimension
        (int)   q           -- Modulus
        (ND)    Xs          -- Secret distribution
        (ND)    Xe          -- Error distribution
        (int)   m           -- Number of NTRU samples (forced to n anyways in estimator)
        (str)   ntru_type   -- NTRU type
        (int)   it          -- Number of calls to SVP oracle [Optional]
        (str)   cost_model  -- SVP oracle modeling [Optional]
    - output: 
        (int)   b           -- BKZ block size
        ...
        (flt)   qsec        -- Quantum bit security
    """
    # Checking if overstretched
    (low,up) = min(Xs, Xe).bounds # bounds of distribution with smallest std. dev.
    is_ternary = (-1 <= low) and (up <= 1)
    if q > n ** 2.484 / exp(6):
        print('[Warning] Parameters in overstretched regime%s. DSD attacks may be more efficient than usual lattice reduction' % ('' if is_ternary else ' (although keys are not ternary)'))

    # Running estimator
    if rough:
        results = NTRU.estimate.rough(NTRU.Parameters(n=n, q=q, Xs=Xs, Xe=Xe, m=m, ntru_type=ntru_type))
    else:
        results = NTRU.estimate(NTRU.Parameters(n=n, q=q, Xs=Xs, Xe=Xe, m=m, ntru_type=ntru_type))
